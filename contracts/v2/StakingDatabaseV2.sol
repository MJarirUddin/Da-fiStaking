...
contract StakingDatabaseV2 is Ownable{
    ...
    function getFirstRebaseTime() external view returns(uint) {
        return firstRebaseTime;
    }

    function setFirstRebaseTime() external onlyWhitelist {
        firstRebaseTime = block.timestamp;
    }
    ...
    function addToTotalFeesCollected(uint _newFees) external onlyWhitelist {
        totalFeesCollected += _newFees;
    }
    ...
    function getAccumulatedPoolWeight() external view returns (uint){
        return pool.currentAccumulatedWeight;
    }

    function getAccumulatedFeeWeight() external view returns(uint) {
